    ...
    <div class="col-8 offset-2">
        <div class="card ">
            <div class="card-header bg-info">
              <p class="text-light font-weight-bold text-center"> Insurance Add Form.</p>
            </div>
            <div class="row">
                <div class="col-md-12">
             @if(session('message'))
            ...
            <div class="card-body">
                <form action=" {{ route('admin.insurance.store') }} " method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror"  id="name" name="name"   value="{{ old('name') }}" placeholder="Enter Insurance Name">
                        @error('name') <span class="text-danger font-italic">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                        <label for="company_name">Company Name</label>
                        <input type="text" class="form-control @error('company_name') is-invalid @enderror"  id="company_name" name="company_name"   value="{{ old('company_name') }}" placeholder="Enter  Insurance Company's Name">
                        @error('company_name') <span class="text-danger font-italic">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                        <label for="insurance_fee">Insurance Fee</label>
                        <input type="text" class="form-control @error('insurance_fee') is-invalid @enderror"  id="insurance_fee" name="insurance_fee"   value="{{ old('insurance_fee') }}" placeholder="Enter  Insurance Fee">
                        @error('insurance_fee') <span class="text-danger font-italic">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                        <label for="coverage">Coverage</label>
                        <textarea name="coverage" id="coverage" placeholder="Give about insurance coverage" class="form-control" value="{{ old('coverage') }}"></textarea>
